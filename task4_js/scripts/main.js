...
function getData() {
  ...
  //fetch ('./scripts/amazing.json')
    ...
    .then((apiData) => {
      ...
      pushCards(events, eventsContainer);
      getCategory(events, checkboxes);
      console.log(events);
      checkboxEvents('input[type="checkbox"]');
      //searchEvents('input[type="search"]');
    })
...
function filterCheck(e) {
 ...
 return  pushCards(events, eventsContainer)
  }
...
function filterSearch(e) {
  ...
  let filteredEvents = events.filter(event => event.name.toLocaleLowerCase().startsWith(result) ||
