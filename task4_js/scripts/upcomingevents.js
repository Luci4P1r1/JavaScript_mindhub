...
function getData() {
  ...
  //fetch ("./scripts/amazing.json")
    ...
    .then((apiData) => {
      ...
      console.log(apiData);
      pushCards(upcomingEvents, eventsContainer);
      getCategory(upcomingEvents, checkboxes);
...
function filterCheck(e) {
  ...
  let filtered = upcomingEvents.filter(event => selectedCheck.includes(event.category))
  console.log(filtered)
  if(selectedCheck.length > 0){
    return pushCards(filtered, eventsContainer)
  }
  else{
 return  pushCards(upcomingEvents, eventsContainer)
  }
...
function filterSearch(e) {
  ...
  let filteredEvents = upcomingEvents.filter(event => event.name.toLocaleLowerCase().startsWith(result) ||
